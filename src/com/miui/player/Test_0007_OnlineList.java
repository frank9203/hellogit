...
public class Test_0007_OnlineList extends UiAutomatorTestCase {
	public Marmot mm;
	public Checker cc;
	...
	public void test_Online_List() throws UiObjectNotFoundException {
		if(!mm.isScreenOn()){
			mm.wakeUp();
			if(device_type==2){
		...
		if (connection == 5){
			...
			if (connection == 5){
				...
				mm.pressMenu();
				found_FC("在线专辑里点击menu");
				mm.waitFor(1);
				mm.pressMenu();
				UiObject index0 = new UiObject(new UiSelector().className("android.widget.ListView"))
				...
				index0.click();
				found_FC("点击收藏在线专辑");
				if( index0.isSelected() ){
					index0.click();
					found_FC("点击取消收藏在线专辑");
					
					mm.log("Step 6 : Enter EditMode.");
					if(device_type==2){
						mm.getObjectByClass("android.widget.ListView").longClick();
					}else if(device_type == 3){
						mDevice.swipe(540, 1200, 540, 1200, 400);
					}
					found_FC("在线专辑内编辑模式");
					mm.clickOnButton("播放");
					found_FC("播放在线专辑的一首歌曲");
					mm.waitFor(1);
					mm.pressBack(4);
					mm.log("Step 7 : End.");
				}else{
